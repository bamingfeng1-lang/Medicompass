...
const CATEGORY_KEY = "basic-medical";

export function generateMetadata({ params }: { params: { lang: string } }): Metadata {
  const lang = isLocale(params.lang) ? params.lang : "zh";
  const t = getDictionary(lang);
  return { title: t.nav.basicMedical };
}

export default function BasicMedicalPage({ params }: { params: { lang: string } }) {
  ...
  return (
    <>
      <section className="relative overflow-hidden bg-brand-950 text-white">
        ...
        <div className="container-page relative py-20 sm:py-28">
          <div className="max-w-3xl animate-fade-up">
            <span className="eyebrow border-white/20 bg-white/10 text-brand-sky">{t.nav.basicMedical}</span>
            <h1 className="mt-6 text-4xl font-bold leading-tight tracking-tight sm:text-5xl lg:text-6xl">
              {lang === "zh" ? "基础医疗" : "Basic Medical"}
            </h1>
            <p className="mt-6 text-lg leading-relaxed text-slate-300">
              {lang === "zh"
                ? "提供从预约就诊到住院陪护的全流程基础医疗服务，让您的就医体验更加便捷、舒适。"
                : "End-to-end basic medical services from appointment to inpatient care — making healthcare access seamless and comfortable."}
            </p>
            <Link
              href={p("/second-opinion/apply?category=" + encodeURIComponent(lang === "zh" ? "基础医疗" : "Basic Medical"))}
              className="btn mt-9 bg-white text-brand-deep hover:bg-brand-50"
            >
              {lang === "zh" ? "立即申请基础医疗" : "Apply for Basic Medical"}
      ...
      <Section className="bg-white">
        <SectionHeading
          title={lang === "zh" ? "基础医疗" : "Basic Medical"}
          desc={lang === "zh"
            ? "提供从预约就诊到住院陪护的全流程基础医疗服务，让您的就医体验更加便捷、舒适。"
            : "End-to-end basic medical services from appointment to inpatient care."}
        />
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {list.map((svc, i) => {
            const copy = lang === "en" ? svc.en : svc.zh;
            const href = svc.external ? p(svc.external) : p(`/services/${svc.slug}`);
            const applyHref = p(`/second-opinion/apply?category=${encodeURIComponent(lang === "zh" ? "基础医疗" : "Basic Medical")}&need=${encodeURIComponent(copy.name)}`);
            return (
