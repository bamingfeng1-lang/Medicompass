...
const CATEGORY_KEY = "cross-border";

export function generateMetadata({ params }: { params: { lang: string } }): Metadata {
  const lang = isLocale(params.lang) ? params.lang : "zh";
  const t = getDictionary(lang);
  return { title: t.nav.crossBorderMedical };
}

export default function CrossBorderMedicalPage({ params }: { params: { lang: string } }) {
  ...
  return (
    <>
      <section className="relative overflow-hidden bg-brand-950 text-white">
        ...
        <div className="container-page relative py-20 sm:py-28">
          <div className="max-w-3xl animate-fade-up">
            <span className="eyebrow border-white/20 bg-white/10 text-brand-sky">{t.nav.crossBorderMedical}</span>
            <h1 className="mt-6 text-4xl font-bold leading-tight tracking-tight sm:text-5xl lg:text-6xl">
              {lang === "zh" ? "跨境国际医疗" : "Cross-border Medical"}
            </h1>
            <p className="mt-6 text-lg leading-relaxed text-slate-300">
              {lang === "zh"
                ? "链接全球顶尖医疗资源，为您提供跨境就医、国际二诊、全球找药等一站式国际医疗服务。"
                : "Connecting you to world-class medical resources — one-stop international healthcare including cross-border treatment, second opinions, and global medication access."}
            </p>
            <Link
              href={p("/second-opinion/apply?category=" + encodeURIComponent(lang === "zh" ? "跨境国际医疗" : "Cross-border Medical"))}
              className="btn mt-9 bg-white text-brand-deep hover:bg-brand-50"
            >
              {lang === "zh" ? "立即申请跨境国际医疗" : "Apply for Cross-border Medical"}
      ...
      <Section className="bg-white">
        <SectionHeading
          title={lang === "zh" ? "跨境国际医疗" : "Cross-border Medical"}
          desc={lang === "zh"
            ? "链接全球顶尖医疗资源，为您提供跨境就医、国际二诊、全球找药等一站式国际医疗服务。"
            : "Connecting you to world-class medical resources — one-stop international healthcare."}
        />
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {list.map((svc, i) => {
            const copy = lang === "en" ? svc.en : svc.zh;
            const href = svc.external ? p(svc.external) : p(`/services/${svc.slug}`);
            const applyHref = p(`/second-opinion/apply?category=${encodeURIComponent(lang === "zh" ? "跨境国际医疗" : "Cross-border Medical")}&need=${encodeURIComponent(copy.name)}`);
            return (
