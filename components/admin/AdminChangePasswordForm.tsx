...
export function AdminChangePasswordForm({ dict }: { dict: Dictionary }) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (busy) return;
    if (!oldPw || !newPw || !confirmPw) { setMsg({ kind: "err", text: a.pwMissing }); return; }
    if (newPw.length < 8) { setMsg({ kind: "err", text: a.pwWeak }); return; }
    if (newPw !== confirmPw) { setMsg({ kind: "err", text: a.pwMismatch }); return; }
    setBusy(true);
    setMsg(null);
    try {
      const res = await fetch(clientApi("/api/admin/change-password"), {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        const text =
          code === "old_password_wrong" ? a.pwOldWrong
          : code === "weak_password" ? a.pwWeak
          : code === "same_password" ? a.pwSame
          : a.pwError;
